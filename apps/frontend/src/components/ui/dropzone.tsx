...
import { type DropzoneProps as _DropzoneProps, DropzoneState, useDropzone } from 'react-dropzone';
import truncate from 'truncate';

// Typescript:
import { cn } from '@/lib/utils';
import { TPseudoFile } from '@/pages/BotBuilder/BotBuilder';
...
export interface DropzoneProps extends Omit<_DropzoneProps, 'children'> {
    ...
    filesUploaded: File[];
    prevExistingFiles?: TPseudoFile[];
    setFilesUploaded: React.Dispatch<React.SetStateAction<File[]>>;
    deletePrevFile: (file: TPseudoFile) => void;
}
...
const UploadedFiles = (
    {
        ...
        showFilesList: boolean;
        filesUploaded: (TPseudoFile | File)[];
        deleteUploadedFile?: (index: number) => void;
        deletePrevFile?: (file: TPseudoFile) => void;
    }
) => (showFilesList && filesUploaded.length > 0) && (
    <div className={`flex flex-col gap-2 w-full mt-2 ${filesUploaded.length > 0 ? 'pb-2' : ''}`}>
        <div className='w-full'>
            {
                filesUploaded.map((fileUploaded, index) => (
                    <div key={fileUploaded.name} className='flex justify-between items-center flex-row w-full h-16 mt-2 px-4 border-solid border-2 border-gray-200 rounded-lg shadow-sm'>
                        ...
                        <div
                            ...
                            onClick={() => {
                                ...
                                deletePrevFile?.(fileUploaded as TPseudoFile);
                            }}
                        >
    ...
    return (
        ...
        >
            <div
                ...
                className={cn('flex justify-center items-center w-full h-10 border-dashed border-2 border-gray-200 rounded-lg hover:bg-accent hover:text-accent-foreground transition-all select-none cursor-pointer', dropZoneClassName)}
            >
            ...
            <div className="overflow-y-scroll h-[40vh]">
                ...
                <UploadedFiles
                    ...
                    filesUploaded={prevExistingFiles}
